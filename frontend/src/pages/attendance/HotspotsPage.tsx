...
export default function HotspotsPage() {
  ...
  const createHotspot = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['hotspots'] }); setShowAddHotspot(false); setForm({ name: '', ssid: '', bssid: '', locationName: '' }); toast.success('Hotspot created'); },
  });

  const deleteHotspot = useMutation({
    mutationFn: (id: string) => attendanceApi.deleteHotspot(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['hotspots'] }); toast.success('Hotspot deleted'); },
  });

  const createFence = useMutation({
    mutationFn: (data: object) => attendanceApi.createGeoFence(data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['geofences'] }); setShowAddFence(false); setFenceForm({ name: '', latitude: '', longitude: '', radius: '200', address: '' }); toast.success('GeoFence created'); },
  });
  ...
  const assignHotspot = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['hotspots'] }); toast.success('Assigned to employee'); setSelectedEmployee(''); },
  });

  const removeHotspot = useMutation({
    mutationFn: ({ empId, id }: { empId: string; id: string }) => attendanceApi.removeHotspot(empId, id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['hotspots'] }); toast.success('Removed from employee'); },
  });

  const assignFence = useMutation({
    mutationFn: ({ empId, id }: { empId: string; id: string }) => attendanceApi.assignGeoFence(empId, id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['geofences'] }); toast.success('Assigned to employee'); setSelectedEmployee(''); },
  });

  const removeFence = useMutation({
    mutationFn: ({ empId, id }: { empId: string; id: string }) => attendanceApi.removeGeoFence(empId, id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['geofences'] }); toast.success('Removed from employee'); },
  });
